...
{	
	...
	{		
		...
		TweenLite.to(textPlate.animText[i], 0.1,{delay: (textPlate.animText.length-i)/8, x:textPlate.animText[i].sx-60, alpha:0, ease:Circ.easeIn});				
	}
	TweenLite.delayedCall(textPlate.animText.length/8+0.2, function ()
	{
		...
		for(var i=0;i<textArray.length;i++)
		{
			...
			textPlate.textArray[textArray[i]].x = textPlate.textArray[textArray[i]].sx+100;
			
			
			TweenLite.to(textPlate.textArray[textArray[i]], 0.2,{delay: i/8, x:textPlate.textArray[textArray[i]].sx, alpha:1, ease:Circ.easeOut});		
			if(delay)
			{
				TweenLite.to(textPlate.textArray[textArray[i]], 0.1,{delay: (textArray.length-i)/8+delay, x:textPlate.textArray[textArray[i]].sx-60, alpha:0, ease:Circ.easeIn});
			}
			textPlate.animText.push(textPlate.textArray[textArray[i]]);
		}		
	});	
...
function hideText()
{
	for(var i=0;i<this.animText.length;i++)
	{		
		TweenLite.killTweensOf(this.animText[i]);		
		TweenLite.to(this.animText[i], 0.1,{delay: (this.animText.length-i)/8, x:this.animText[i].sx-60, alpha:0, ease:Circ.easeIn});
	}
}
...
{	
	TweenLite.to(this, 0.2, {y: this.sy,  ease:Sine.easeOut});	
}
...
{
	TweenLite.to(this, 0.2, {y: h+this.height + timerImg.height + 6,  ease:Sine.easeOut});
}
